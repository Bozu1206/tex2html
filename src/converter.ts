...
async function preprocessFile(texFilePath: string, outputFormat: string, context: vscode.ExtensionContext, progressCallback?: (message: string, increment?: number) => void) {
    ...
    const ret = await execCommand(`"${pythonConfig}" "${scriptPath}" "${texFilePath}" ${outputFormat}`);
...
export async function convertTexToHtml(texFilePath: string, context: vscode.ExtensionContext) {
    ...
    const pandocCommand = `"${pandocConfig}" -s "${tempFilePath}" -M link-citations=true ${bib_engine !== "?" && bib_filename !== "?" ? `--bibliography="${bib_file_path}" --citeproc --csl="${cslFilePath}"` : ''} --metadata lang="${lang}" --metadata reference-section-title="${ref}" --mathjax --include-in-header "${headerFilePath}" -t html -N -o "${htmlFilePath}"`;
...
export async function convertTexToPDF(texFilePath: string, context: vscode.ExtensionContext) {
    await vscode.window.withProgress({
        ...
        if (bib_engine !== "?" && bib_filename !== "?") {
            ...
            const latexConfig = vscode.workspace.getConfiguration().get('tex2HTML.latex.commandForLatex');
            const latexCommand = `"${latexConfig}" -output-directory="${path.dirname(texFilePath)}" -jobname="${pdf}" "${tempFilePath}"`;
            ...
            await execCommand(`pushd "${path.dirname(texFilePath)}" && ${effectiveBibEngine} "${pdf}" && popd`);
